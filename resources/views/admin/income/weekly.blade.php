    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-gray-600">{{ \Carbon\Carbon::create($year, $month)->format('F Y') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-r from-[#7B542F] to-[#A0724B] rounded-2xl p-6 text-white mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-xl font-bold mb-2">Total Monthly Income</h3>
                    <p class="text-3xl font-bold">Rp {{ number_format($totalMonth, 0, ',', '.') }}</p>
                </div>
                <div class="text-right">
                    <p class="text-sm opacity-90">Month: {{ \Carbon\Carbon::create($year, $month)->format('F Y') }}</p>
                    <p class="text-sm opacity-90">{{ count($weeklyData) }} weeks</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-100 hover:shadow-xl transition-all duration-300">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-bold text-gray-900">{{ $week['week'] }}</h3>
                ...
                <div class="mt-4">
                    <div class="flex justify-between text-xs text-gray-600 mb-1">
                        <span>Progress</span>
                        <span>{{ $totalMonth > 0 ? round(($week['income'] / $totalMonth) * 100, 1) : 0 }}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="bg-gradient-to-r from-[#7B542F] to-[#A0724B] h-2 rounded-full transition-all duration-500"
                             style="width: {{ $totalMonth > 0 ? ($week['income'] / $totalMonth) * 100 : 0 }}%"></div>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="mt-8 bg-white rounded-2xl shadow-md p-6 border border-gray-100">
            <h3 class="text-xl font-bold text-gray-900 mb-6">Weekly Income Chart</h3>
            <div class="h-64">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Income (Rp)',
                data: weeklyData.map(week => week.income),
                backgroundColor: 'rgba(123, 84, 47, 0.8)',
