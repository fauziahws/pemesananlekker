    ...
    <div class="bg-white/70 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/50">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center">
                    <input type="checkbox" name="remember" class="w-4 h-4 text-[#7B542F] border-gray-300 rounded focus:ring-[#7B542F]">
                    <span class="ml-2 text-sm text-gray-600">Remember me</span>
                </label>

                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-sm text-[#7B542F] hover:text-[#5D3F22] font-medium transition">
                        Forgot password?
                    </a>
                @endif
            </div>
        ...
        <div class="mt-6 text-center">
            <p class="text-sm text-gray-600">
                Belum punya akun? 
                <a href="{{ route('register') }}" class="font-semibold text-[#7B542F] hover:text-[#5D3F22] transition">
                    Daftar Sekarang
                </a>
            </p>
        </div>
