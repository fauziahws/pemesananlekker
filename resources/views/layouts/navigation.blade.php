        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('menu.index') }}" class="text-xl font-bold text-gray-800">
                        Lekker
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @elseif(auth()->user()->isCashier())
                        ...
                        <x-nav-link :href="route('cashier.orders.all')" :active="request()->routeIs('cashier.orders.all')">
                            All Orders
                        </x-nav-link>
                    @else
                        ...
                        <x-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.index')">
                            <div class="flex items-center gap-2">
                                ...
                                @endphp
                                @if($cartCount > 0)
                                    <span id="cart-count" class="bg-red-500 text-white text-xs font-bold rounded-full px-2 py-0.5">{{ $cartCount }}</span>
                                @else
                                    <span id="cart-count" class="bg-gray-300 text-gray-600 text-xs font-bold rounded-full px-2 py-0.5">0</span>
                                @endif
                            </div>
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @elseif(auth()->user()->isCashier())
                ...
                <x-responsive-nav-link :href="route('cashier.orders.all')" :active="request()->routeIs('cashier.orders.all')">
                    All Orders
                </x-responsive-nav-link>
            @else
                ...
                <x-responsive-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.index')">
                    Cart (@php echo session()->has('cart') ? array_sum(array_column(session('cart'), 'quantity')) : 0; @endphp)
                </x-responsive-nav-link>
